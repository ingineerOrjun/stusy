...
/* generic animated step player.
   ...
   Pacing goes through MotionService so a JavaScript-driven sequence obeys
   prefers-reduced-motion, which a CSS media query cannot reach.

   Under reduced motion the whole sequence is played out at once: every
   caption and every output line is written immediately and the final state
   is rendered. The student loses the animation but loses no information —
   the console is a log, so reading it complete is equivalent. That is the
   correct reduced-motion behaviour here: less movement, not less teaching. */
function runSteps(codeElId, codeLines, steps, conEl, guard, onDone){
  if (guard.busy) return;
  if (!document.getElementById(codeElId)) return;
  guard.busy = true;

  var reduced = (typeof MotionService !== 'undefined') && MotionService.prefersReduced();
  var pace = (typeof MotionService !== 'undefined') ? MotionService.ms('step') : 760;
  ...
  if (reduced){
    for (var k = 0; k < steps.length; k++) render(steps[k]);
    guard.busy = false;
    if (onDone) onDone();
    return;
  }
...
(function(){
  ...
  ready(function(){
    ...
    /* ---- scroll affordance for wide content ----
       A diagram or table cropped at the viewport edge reads as broken
       unless the student is told it scrolls. Marked only when the content
       genuinely overflows, and re-checked on resize. */
    function markScrollable(){
      var wide = document.querySelectorAll('figure.fig, .tablewrap, .code, .dia-stage');
      for (var i = 0; i < wide.length; i++){
        var el = wide[i];
        var over = el.scrollWidth > el.clientWidth + 2;
        el.classList.toggle('is-scrollable', over);
        if (over && !el.getAttribute('tabindex')){
          /* a keyboard user must be able to scroll it too */
    ...
    window.addEventListener('resize', function(){
      clearTimeout(rt); rt = setTimeout(markScrollable, 150);
    });

    /* ---- "show the answer" toggles on practice questions ----
       The button controls a region, so it must say so: without
       aria-expanded a screen-reader user cannot tell whether the model
       answer is currently shown. */
    ...
    /* ---- simulator output must be announced ----
       A student using a screen reader presses push() and, without this,
       hears nothing at all: the console updates silently. politeness =
    ...
    for (var c = 0; c < consoles.length; c++){
      consoles[c].setAttribute('role', 'log');
